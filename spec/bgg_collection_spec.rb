...
describe BggCollection do
  describe 'class method' do
    describe 'find_by_username' do
      ...
      it 'throws an ArgumentError when a non-existent user is passed in' do
        ...
        expect{ BggCollection.find_by_username('yyyyyyy')}.to raise_error(ArgumentError)
      end
      ...
      it 'creates an object for a collection that exists' do
        ...
        collection = BggCollection.find_by_username('texasjdl')

        expect( collection ).to be_a_kind_of(BggCollection)
  ...
  describe 'instance' do
    let(:collection) { BggCollection.find_by_username('texasjdl') }
    ...
    it 'has boardgames' do
      ...
      expect( collection.boardgames.first ).to be_instance_of(BggCollectionItem)
    ...
    it 'has boardgame_expansions' do
      ...
      expect( collection.boardgame_expansions.first ).to be_instance_of(BggCollectionItem)
      expect( collection.boardgame_expansions.count).to eq(37)
    end

    it 'has played' do
      expect( collection.played ).to be_instance_of(Array)
      expect( collection.played.first ).to be_instance_of(BggCollectionItem)
